...
export default function AdminPreferences() {
  ...
  const fetchPreferences = async () => {
    setError('');
    try {
      const res = await preferencesAPI.getPreferences('admin');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      await preferencesAPI.updatePreferences('admin', preferences);
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 flex justify-center items-start py-12">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg">
          <h2 className="text-2xl font-bold mb-6 text-gray-900">Admin Preferences</h2>
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full bg-orange-600 text-white py-3 px-4 rounded-lg font-semibold hover:bg-orange-700 disabled:opacity-50"
            >
